...
class RadicalTranslationHanziQuestionGenerator(QuestionGenerator):
    ...
    def generate(self) -> list[Question]:
        ...
        for word in self._radicals_dictionary.get_radicals_list():
            question_type = QuestionType.RADICAL_TRANSLATION_HANZI
            ...
            if existing_question:
                ...
                if len(word.translations) == 1:
                    mistake_details = f"Перевод для данного ключа - {word.translations[0]}"
                else:
                    mistake_details = f"Варианты перевода для данного ключа - {", ".join(word.translations)}"
                question = Question(
                question_type=question_type,
                word_id=word.word_id,
                html=html_template.format(number=word.number, hanzi=word.hanzi, pinyin=word.pinyin),
